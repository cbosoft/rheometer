...
void usage(void)
{
  fprintf(stdout, 
      ...
      "    rheometer run -l <length> -d <fill-depth> (-c <control scheme> | -c <controller> -s <setter>) -w <hardware version> [OPTIONAL: see below]\n"
      "    rheometer schedule [-n <interp_number>] [-c <controller>]... [-s <setter>]... [-p <params>]... [-a]... [run options] \n"
...
void args_help()
{
  fprintf(stdout,
      ...
      "    --quiet          suppress most output; progress is shown by progress bar.\n"
      "\n"
      ...
      "\n"
      "    -c | --controller    Controller module name.\n"
      "\n"
      "    -s | --controller    Setter module name.\n"
      "\n"
      "    -pc | --controller-params    Parameters passed to controller.\n"
      "\n"
      "    -ps | --setter-params    Parameters passed to setter.\n"
      "\n"
      "    -j | --control-scheme    Control scheme JSON file path. More information in the\n"
      ...
      "\n"
      "    -v | --video     By setting this option, video logging is enabled. A video of the name\n"
      "                     \"${prefix}_video.mp4\" will be created and stored alongside the other\n"
      "                     log files. Uses RPi camera module.\n"
      "\n"
      "    -p | --photo     By setting this option, a photo of the name \"${prefix}_photo.mp4\"\n"
      "                     will be created and stored alongside the other log files, taken before\n"
      "                     the logging begins. Uses RPi camera module.\n"
      "\n"
      ...
      "\n"
      "    --interp-number  Number of interpolation points to use.\n"
      "\n"
      "    --interp-linear  Use linear interpolation.\n"
      "\n"
      "    --interp-log     Use logarithmic interpolation.\n"
      "\n"
  ); // TODO: config options
}
